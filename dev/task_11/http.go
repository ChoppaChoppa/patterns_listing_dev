...
type (
	User struct {
		...
		Calendar map[time.Time][]string `json:"calendar,omitempty"`
	}
...
func CreateAction(data Data) (User, error) {
	t, errParse := time.Parse(shortFormDate, data.Date)
	if errParse != nil {
...
func (d *Data) parseDate() (time.Time, error) {
	t, errParse := time.Parse(shortFormDate, d.Date)
	if errParse != nil {
...
func parseUrl(value url.Values) (int, time.Time, error) {
	...
	t, errParseDate := time.Parse(shortFormDate, dateStr)
	if errParseDate != nil {
		return -1, time.Time{}, errParseID
	}
